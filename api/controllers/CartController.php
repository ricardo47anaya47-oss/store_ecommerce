...
class CartController {
    ...
    public function __construct() {
        ...
        $this->ensureCartTables();
    }

    private function ensureCartTables() {
        $cartTable = $this->db->query("SHOW TABLES LIKE 'cart'");
        if (!$cartTable || $cartTable->num_rows === 0) {
            $this->db->query(
                "CREATE TABLE cart (
        ...
        $detailTable = $this->db->query("SHOW TABLES LIKE 'cart_detail'");
        if (!$detailTable || $detailTable->num_rows === 0) {
            $this->db->query(
