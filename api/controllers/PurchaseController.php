...
class PurchaseController {
    ...
    public function __construct() {
        ...
        $this->ensurePurchaseTables();
    }

    private function ensurePurchaseTables() {
        $purchaseTable = $this->db->query("SHOW TABLES LIKE 'purchase'");
        if (!$purchaseTable || $purchaseTable->num_rows === 0) {
            $this->db->query(
        ...
        $purchaseDetailTable = $this->db->query("SHOW TABLES LIKE 'purchase_detail'");
        if (!$purchaseDetailTable || $purchaseDetailTable->num_rows === 0) {
            $this->db->query(
